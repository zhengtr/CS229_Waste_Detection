"""
Original file from  https://github.com/pedropro/TACO
Small changes by Gabriela B.
"""
...
parser.add_argument('--json_file', required=True, help='name.json of file to split')
...
ann_input_path = args.dataset_dir + '/' +  args.json_file
...
nr_nontraining_images = int(nr_images*(args.val_percentage)*0.01+0.5)

#define a priory test set, same for all trials
random.shuffle(imgs)

images_test= imgs[0:nr_testing_images]
images_val_train = imgs[nr_testing_images:nr_images]
nr_images_val_train = len(images_val_train)


for i in range(args.nr_trials):
    random.shuffle(images_val_train)
   ...
   # train_set['scene_categories'] = dataset['scene_categories']
    ...
    test_set['images'] = images_test
    val_set['images'] = images_val_train[0:nr_nontraining_images]
    train_set['images'] = images_val_train[nr_nontraining_images:nr_images_val_train]
